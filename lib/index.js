...
async function validatePullRequest(octokit, requirements) {
    ...
    const datesBySource = dateReqs.reduce((acc, req) => {
        if (!acc[req.source]) {
            acc[req.source] = [];
        }
    ...
    if (mostRestrictiveDates.length > 0) {
        const highestPriorityDate = mostRestrictiveDates.reduce((prev, current) => current.priority > prev.priority ? current : prev);
        ...
        if (now < targetDate) {
            ...
            result.reasons.push(`Cannot merge before ${highestPriorityDate.value} (${Math.ceil((targetDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))} days remaining)`);
        }
    }
    // For dependencies, we combine all of them
    for (const dep of dependencyReqs) {
        try {
            const { owner, repo, pullNumber } = (0, url_parser_1.parsePullRequestUrl)(dep.value);
